...
if __name__ == '__main__':
    ...
    if args.dataset != None:
        config['dataloader']['dataset']['name'] = args.dataset
        config['dataloader']['dataset']['data_root'] = data_loadpath[args.dataset]['loadpath']
        config['dataloader']['dataset']['keep_first'] = data_loadpath[args.dataset]['keep_first']
        config['dataloader']['dim'] = data_loadpath[args.dataset]['dim']
        config['model']['common']['input_dim'] = data_loadpath[args.dataset]['dim']
    if args.lf_path != None:
        config['exp']['biddpm']['ddpm_lf']['load_path'] = args.lf_path
    print(f"[START] training on GPU:{args.gpu} W{config['dataloader']['dataset']['window']} {config['dataloader']['dataset']['name']}")
    ...
    config['exp']['lab_name'] = f"{config['exp']['lab_name']}_{config['dataloader']['dataset']['name']}_W{config['dataloader']['dataset']['window']}_{current_date}"
    ...
    if 'load_path' not in config['exp']:
        ...
        with torch.no_grad():
            for i in tqdm(test_dataloader):
                x_i = torch.tensor(i, dtype=torch.float32).to(device)  
